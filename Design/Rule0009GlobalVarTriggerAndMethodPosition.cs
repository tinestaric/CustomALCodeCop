...
{
    [DiagnosticAnalyzer]
    public class Rule0009GlobalVarTriggerAndMethodPosition : DiagnosticAnalyzer
    {
        public override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics { get; } = ImmutableArray.Create<DiagnosticDescriptor>(DiagnosticDescriptors.Rule0009GlobalVarTriggerAndMethodPosition);
        ...
        private void AnalyzeGlobalVariablesPlacement(SyntaxNodeAnalysisContext ctx)
        {
            ...
            if ((globalVarSectionPosition > triggersSectionPosition && triggersSectionPosition != 0) || (globalVarSectionPosition > methodSectionPosition && methodSectionPosition != 0))
                ctx.ReportDiagnostic(Diagnostic.Create(DiagnosticDescriptors.Rule0009GlobalVarTriggerAndMethodPosition, globalVarLocation));

            if (triggersSectionPosition > methodSectionPosition && methodSectionPosition != 0)
                ctx.ReportDiagnostic(Diagnostic.Create(DiagnosticDescriptors.Rule0009GlobalVarTriggerAndMethodPosition, triggerLocation));
        }
    }
}
