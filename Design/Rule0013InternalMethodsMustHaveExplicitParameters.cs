...
{
    [DiagnosticAnalyzer]
    public class Rule0013InternalMethodsMustHaveExplicitParameters : DiagnosticAnalyzer
    {
        public override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics { get; } = ImmutableArray.Create<DiagnosticDescriptor>(DiagnosticDescriptors.Rule0013InternalMethodsMustHaveExplicitParameters);
        ...
        private void CheckCommitForExplainingComment(OperationAnalysisContext ctx)
        {
            ...
            if (methodsToCheck.Contains(targetMethod.Name.ToUpper()))
                if (operation.Arguments.Length == 0)
                    ctx.ReportDiagnostic(Diagnostic.Create(DiagnosticDescriptors.Rule0013InternalMethodsMustHaveExplicitParameters, ctx.Operation.Syntax.GetLocation()));        
        }
    }
}