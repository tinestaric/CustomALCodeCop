...
{
    [DiagnosticAnalyzer]
    public class Rule0012CommitMustBeExplainedByComment : DiagnosticAnalyzer
    {
        public override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics { get; } = ImmutableArray.Create<DiagnosticDescriptor>(DiagnosticDescriptors.Rule0012CommitMustBeExplainedByComment);
        ...
        private void CheckCommitForExplainingComment(OperationAnalysisContext ctx)
        {
            ...
            if (operation.TargetMethod.Name.ToUpper() == "COMMIT")
            {
                ...
                ctx.ReportDiagnostic(Diagnostic.Create(DiagnosticDescriptors.Rule0012CommitMustBeExplainedByComment, ctx.Operation.Syntax.GetLocation()));
